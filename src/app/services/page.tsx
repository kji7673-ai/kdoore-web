  ...
  const services = [
    {
      ...
      scopes: [
        { title: '출입 통제', items: ['상주 근무자 및 내방객 출입 확인', '물품 반출입 통제 및 기록', '잡상인 및 위험인물 출입 차단'] },
        { title: '방범 및 순찰', items: ['주·야간 정기 및 수시 순찰', '도난, 화재 등 사고 예방', 'CCTV 모니터링 및 이상징후 파악'] },
        { title: '긴급 대응', items: ['비상사태 발생 시 초기 진압', '경찰, 소방 등 유관기관 신속 연락체계 가동', '상황 종료 후 재발방지 보고서 작성'] }
      ]
    },
    {
      id: '1626228350',
      ...
      scopes: [
        { title: '전기/기계 설비', items: ['수배전반, 발전기, 승강기 정기 점검', '공조, 냉난방, 보일러 설비 최적화 운전', '에너지 사용량 모니터링 및 절감 컨설팅'] },
        { title: '소방 및 방재', items: ['화재 수신기 및 소화 펌프 상시 점검', '법정 소방 점검 대행 및 지적사항 보수', '화재 예방 시뮬레이션 훈련'] },
        { title: '건축물 유지보수', items: ['외벽, 지붕, 방수 상태 점검', '내부 마감재 및 구조물 하자 보수', '법정 시설물 안전 진단 지원'] }
      ]
    },
    {
      id: '1626228355',
      ...
      scopes: [
        { title: '일상 미화', items: ['로비, 복도, 화장실 등 공용부 일일 청소', '사무실 내부 바닥, 쓰레기 분리수거', '엘리베이터 및 계단실 청결 유지'] },
        { title: '특수 청소', items: ['카페트 스팀 세척 및 얼룩 제거', '대리석 연마 광택 및 원목 바닥 코팅', '초미립자 살균 방역 및 냄새 제거'] },
        { title: '외벽 및 준공', items: ['고층 건물 유리창 및 외벽 고압 세척', '신축/리모델링 건물 입주 전 준공 청소', '에어컨 및 환풍기 필터 분해 세척'] }
      ]
    },
    {
      id: '1626228361',
      ...
      scopes: [
        { title: '사무지원', items: ['일반 사무, 행정, 경리 보조 인력', '비서, 리셉션, 안내데스크 요원', '단기 프로젝트 투입 사무 인원'] },
        { title: '콜센터 (CS)', items: ['인바운드/아웃바운드 전문 상담원', '고객 불만 처리 및 해피콜 요원', '텔레마케팅 및 리서치 요원'] },
        { title: '생산/물류', items: ['제조 공정 단순 조립 및 검사 인력', '물류센터 입출고, 포장, 피킹 요원', '시설 식당 조리보조 및 배식 인원'] }
      ]
    },
    {
      id: '1626228413',
      ...
      scopes: [
        { title: '해충 방제', items: ['바퀴벌레, 개미, 쥐 등 혐오 해충 퇴치', '비래해충(모기, 파리) 포충기 설치 및 관리', '서식지 파악 및 약제 분무/독먹이 시공'] },
        { title: '살균 소독', items: ['코로나19 등 바이러스 예방 초미립자(ULV) 분무 살균', '접촉 빈도가 높은 손잡이, 승강기 버튼 집중 표면 소독', '화장실 및 배수구 악취 원인균 살균'] },
        { title: '법정 소독', items: ['감염병의 예방 및 관리에 관한 법률에 따른 의무 소독', '소독 증명서 발급 및 관할 보건소 실적 보고', '수목 소독 및 건물 외부 조경 병해충 방제'] }
      ]
    },
    {
      id: '1626228418',
      ...
      scopes: [
        { title: '반기 정기청소', items: ['수도법에 따른 반기 1회 의무 청소', '수조 내부 슬러지 및 물때 완벽 제거', '벽면 및 바닥 고압 세척'] },
        { title: '수질 관리', items: ['수질 검사 대행 및 성적서 발급', '잔류 염소 측정 및 수질 안전성 확보', '수조 환기 및 내부 소독'] },
        { title: '시설 점검', items: ['수위 조절 밸브 및 펌프류 상태 확인', '맨홀 뚜껑 시건 장치 점검', '오버플로우관 방충망 상태 점검'] }
      ]
    }
  ];
    ...
    <main className="min-h-screen bg-apple-canvas-parchment pt-24 pb-12">
      ...
      <div className="bg-apple-surface-tile-1 text-white py-32 relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 max-w-5xl relative z-10 text-center">
          ...
          <p className="text-apple-lead-airy text-apple-body-muted max-w-2xl mx-auto">
            전문적이고 체계적인 케이두레만의 맞춤형 종합 관리 솔루션.<br/>
            ...
            <div key={svc.id} id={svc.id} className="scroll-mt-32">
              <div className={`flex flex-col lg:flex-row gap-12 lg:gap-16 items-start ${index % 2 !== 0 ? 'lg:flex-row-reverse' : ''}`}>
                ...
                <div className="w-full lg:w-[55%] flex flex-col pt-4">
                  ...
                  <p className="text-apple-body text-apple-ink-muted-80 leading-relaxed mb-10">{svc.details}</p>
      ...
      <section className="py-24 bg-apple-surface-tile-1 text-white text-center">
        <div className="container mx-auto px-6 max-w-3xl">
          <h2 className="text-apple-display-md font-bold mb-6">전문적인 관리가 필요하신가요?</h2>
          <p className="text-apple-lead-airy text-apple-body-muted mb-12">케이두레의 전문가가 귀하의 시설에 최적화된 맞춤형 관리 솔루션과 비교 견적을 제안해 드립니다.</p>
          <div className="flex justify-center">
            <Link href="/about#contact" className="bg-apple-primary hover:bg-apple-primary-focus text-white px-8 py-4 rounded-full text-lg font-medium transition-colors shadow-lg">
              현장 방문 상담 요청하기
            </Link>
          </div>
        </div>
